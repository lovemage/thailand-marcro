...
class CMSLoader {
        ...
        try {
            ...
            for (const fileName of fileList) {
                try {
                    ...
                    // 檢查是否遇到API限制或成功率低
                    if (!apiLimited && (successCount / Math.max(files.length, 1) > 0.3 || files.length < 2)) {
    ...
    async getFileList(folder) {
        ...
        // 回退到硬編碼列表
        const knownFiles = {
            'properties': ['property-1.md', 'property-2.md', 'property-3.md', 'property-4.md', 'property-5.md'],
...
async function initializeCMS() {
    ...
    try {
        ...
        console.log('檢查 YouTube Shorts 容器:');
        ...
        const testSelectors = [
            ...
            '.youtube-shorts-section .row.justify-content-center'
        ];
        ...
        try {
            ...
            // 先快速檢測API是否可用
            const testResponse = await fetch('https://api.github.com/repos/lovemage/thailand-marcro/contents/_data/properties');
            if (testResponse.status === 403) {
                console.warn('GitHub API完全受限制，直接使用回退數據');
                throw new Error('API受限制');
            }
        ...
        let videos = [];
        try {
            videos = await loader.loadYouTubeVideos();
            console.log(`從 CMS 載入了 ${videos.length} 個 YouTube 影片`);
            if (videos.length === 0) {
                throw new Error('CMS 返回空數據');
            }
        } catch (error) {
            console.warn('CMS 影片數據載入失敗，使用回退數據:', error.message);
            videos = fallbackData.videos;
        }

        const videosContainer = document.querySelector('.youtube-section .row');
        if (videosContainer) {
            const loadingEl = videosContainer.querySelector('#videos-loading');
            if (loadingEl) loadingEl.remove();
            videosContainer.innerHTML = generateYouTubeHTML(videos);
            console.log(`載入了 ${videos.length} 個 YouTube 影片`);
        }

        // 載入 YouTube Shorts 數據
        let shorts = [];
        try {
            console.log('開始載入 YouTube Shorts 數據...');
            shorts = await loader.loadYouTubeShorts();
            console.log(`從 CMS 載入了 ${shorts.length} 個 YouTube Shorts:`, shorts);

            // 檢查數據完整性
            const validShorts = shorts.filter(short =>
                short && (short.title || short.filename) &&
                (short.shorts_url || short.video_url)
            );

            if (validShorts.length === 0) {
                throw new Error('CMS 數據無效或為空');
            }

            shorts = validShorts;
            console.log(`過濾後的有效 shorts: ${shorts.length} 個`);

        } catch (error) {
            console.warn('CMS 短影片數據載入失敗，使用回退數據:', error.message);
            shorts = fallbackData.shorts;
            console.log('使用回退數據:', shorts);
        }
        ...
        const shortsSelectors = [
            '.youtube-shorts-section .row.justify-content-center',
            '.youtube-shorts-section .row',
            '.youtube-shorts-section .container .row',
            '#shorts-section .row',
            '.youtube-shorts-section'
        ];
        ...
        for (const selector of shortsSelectors) {
            ...
            if (shortsContainer) {
                console.log(`找到 shorts 容器，選擇器: ${selector}`);
                break;
            }
        }

        if (shortsContainer) {
            const loadingEl = shortsContainer.querySelector('#shorts-loading');
            if (loadingEl) {
                console.log('移除載入指示器');
                loadingEl.remove();
            }

            const generatedHTML = generateYouTubeShortsHTML(shorts);
            console.log('生成的 HTML:', generatedHTML);

            if (shortsContainer.classList.contains('row')) {
                shortsContainer.innerHTML = generatedHTML;
            } else {
                // 如果容器不是 row，找到其中的 row 或創建一個
                const rowContainer = shortsContainer.querySelector('.row') || shortsContainer;
                rowContainer.innerHTML = generatedHTML;
            }

            console.log(`成功載入 ${shorts.length} 個 YouTube Shorts 到頁面`);
        } else {
            console.error('找不到 YouTube Shorts 容器！已嘗試的選擇器:', shortsSelectors);
        }
        ...
        if (typeof $ !== 'undefined' && $.fn.isotope) {
            $('.portfolio_container').isotope('reloadItems').isotope();
        }
        ...
        const videosContainer = document.querySelector('.youtube-section .row');
        if (videosContainer) {
            videosContainer.innerHTML = generateYouTubeHTML(fallbackData.videos);
        }
        ...
        for (const selector of shortsSelectors) {
            shortsContainer = document.querySelector(selector);
            if (shortsContainer) {
                console.log(`回退模式找到 shorts 容器，選擇器: ${selector}`);
                break;
            }
        }
        ...
        if (shortsContainer) {
            if (shortsContainer.classList.contains('row')) {
                shortsContainer.innerHTML = generateYouTubeShortsHTML(fallbackData.shorts);
            } else {
                const rowContainer = shortsContainer.querySelector('.row') || shortsContainer;
                rowContainer.innerHTML = generateYouTubeShortsHTML(fallbackData.shorts);
            }
            console.log('回退模式成功載入 shorts');
        } else {
            console.error('回退模式：找不到 shorts 容器！');
        }
        ...
        $('.work-zoom').magnificPopup({
            ...
            callbacks: {
                open: function() {
                    ...
                    console.log('彈窗元素:', popup.length);

                    popup.attr('data-mode', 'image');

                    const imageView = popup.find('.work-popup-image-view');
                    const textView = popup.find('.work-popup-text-view');
                    console.log('圖片視圖:', imageView.length, '文字視圖:', textView.length);

                    imageView.addClass('active');
                    textView.removeClass('active');

                    console.log('初始化完成 - 圖片模式');

                    // 添加點擊切換事件
                    popup.off('click.toggleMode').on('click.toggleMode', function(e) {
                        // 如果點擊的是關閉按鈕，不執行切換
                        if ($(e.target).hasClass('mfp-close') || $(e.target).closest('.mfp-close').length > 0) {
                            return;
                        }

                        e.stopPropagation();
                        togglePopupMode($(this));
                    });
                }
            }
        });
 ...
 * 切換彈窗顯示模式（圖片/文字）
 */
function togglePopupMode(popup) {
    const currentMode = popup.attr('data-mode');
    const imageView = popup.find('.work-popup-image-view');
    const textView = popup.find('.work-popup-text-view');

    if (currentMode === 'image') {
        // 切換到文字模式
        popup.attr('data-mode', 'text');
