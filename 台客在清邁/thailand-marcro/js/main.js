		...
		$.ajax({
			...
			success : function(text){
	            if (text == "success"){
	            	...
	            	reset_form();
	                			chack.text("您的訊息已成功發送 :)"); // Message Send Text
...
$(document).ready(function() {
    ...
    $(document).click(function(e) {
        if (!$(e.target).closest('.floating-action-button').length) {
            $('.floating-action-button').removeClass('active');
            $('.fab-main').removeClass('rotating');
        }
    });

    // LINE ID click handler
    $('.fab-option').eq(0).click(function() {
        // Copy LINE ID to clipboard
        if (navigator.clipboard) {
            navigator.clipboard.writeText('angra308').then(function() {
                alert('LINE ID: angra308 已複製到剪貼板');
            });
        } else {
            // Fallback for older browsers
            var textArea = document.createElement('textarea');
            textArea.value = 'angra308';
    ...
    // WeChat ID click handler
    $('.fab-option').eq(1).click(function() {
        // Copy WeChat ID to clipboard
        if (navigator.clipboard) {
            navigator.clipboard.writeText('angra308').then(function() {
                alert('WeChat ID: angra308 已複製到剪貼板');
            });
        } else {
            // Fallback for older browsers
            var textArea = document.createElement('textarea');
   ...
   // 聯絡表單中的LINE ID點擊複製功能
   $('.line-id-highlight').click(function() {
       var lineId = $(this).text();
       if (navigator.clipboard) {
           navigator.clipboard.writeText(lineId).then(function() {
               alert('LINE ID: ' + lineId + ' 已複製到剪貼板\n請到LINE搜尋此ID預約看房！');
           });
       } else {
           var textArea = document.createElement('textarea');
